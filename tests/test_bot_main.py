...
async def test_create_session_success(mock_update, mock_context, patch_db):
    ...
    with patch("tmux_ssh_telegram.bot.main.tmux_manager", spec=True) as mock_tmux:
        ...
        with patch("tmux_ssh_telegram.db.repositories.SessionRepository.create", return_value=mock_session):
            with patch("tmux_ssh_telegram.db.repositories.UserRepository.set_active_session", return_value=None):
                await create_session(mock_update, mock_context)
                args, kwargs = mock_update.message.reply_text.call_args
...
async def test_switch_session_not_found(mock_update, mock_context, patch_db):
    ...
    with patch("tmux_ssh_telegram.db.repositories.SessionRepository.get_by_name", return_value=None):
        await switch_session(mock_update, mock_context)
        args, kwargs = mock_update.message.reply_text.call_args
...
async def test_switch_session_success(mock_update, mock_context, patch_db):
    ...
    with patch("tmux_ssh_telegram.db.repositories.SessionRepository.get_by_name", return_value=mock_session):
        with patch("tmux_ssh_telegram.db.repositories.UserRepository.set_active_session", return_value=None):
            await switch_session(mock_update, mock_context)
            args, kwargs = mock_update.message.reply_text.call_args
...
async def test_kill_session_success(mock_update, mock_context, patch_db):
    ...
    with patch("tmux_ssh_telegram.bot.main.tmux_manager", spec=True) as mock_tmux:
        ...
        with patch("tmux_ssh_telegram.db.repositories.SessionRepository.get_by_name", return_value=mock_session):
            with patch("tmux_ssh_telegram.db.repositories.SessionRepository.update_status", return_value=None):
                await kill_session(mock_update, mock_context)
                args, kwargs = mock_update.message.reply_text.call_args
...
async def test_manage_config_show(mock_update, mock_context, patch_db):
    with patch("tmux_ssh_telegram.db.repositories.SettingRepository.get_value", return_value=None):
        await manage_config(mock_update, mock_context)
        args, kwargs = mock_update.message.reply_text.call_args
...
async def test_manage_config_set_success(mock_update, mock_context, patch_db):
    ...
    with patch("tmux_ssh_telegram.db.repositories.SettingRepository.set_value", return_value=None):
        await manage_config(mock_update, mock_context)
        args, kwargs = mock_update.message.reply_text.call_args
...
async def test_manage_config_set_log_level(mock_update, mock_context, patch_db):
    ...
    with patch("tmux_ssh_telegram.db.repositories.SettingRepository.set_value", return_value=None):
        with patch("tmux_ssh_telegram.bot.main.setup_logging") as mock_setup:
            await manage_config(mock_update, mock_context)
...
async def test_manage_config_invalid_key(mock_update, mock_context, patch_db):
    ...
    with patch("tmux_ssh_telegram.bot.main.settings", spec=True) as mock_settings:
        mock_settings.ADMIN_USER_ID = 1
...
async def test_output_monitor_task_exception(mock_update):
    mock_app = MagicMock()
    with patch("tmux_ssh_telegram.bot.main.async_session_factory", side_effect=Exception("DB Error")):
...
async def test_output_monitor_task_no_output(mock_update):
    ...
    with patch("tmux_ssh_telegram.bot.main.async_session_factory") as mock_db:
        mock_session_obj = AsyncMock()
        mock_session_obj.__aenter__.return_value = mock_session_obj
        mock_db.return_value = mock_session_obj
        with patch("tmux_ssh_telegram.db.repositories.SessionRepository.get_active_sessions", return_value=[mock_session]):
            with patch("tmux_ssh_telegram.bot.main.tmux_manager", spec=True) as mock_tmux:
    ...
    with patch("tmux_ssh_telegram.bot.main.async_session_factory") as mock_db:
        ...
        with patch("tmux_ssh_telegram.db.repositories.SessionRepository.get_active_sessions", return_value=[mock_session]):
            with patch("tmux_ssh_telegram.bot.main.tmux_manager", spec=True) as mock_tmux:
                ...
                with patch("tmux_ssh_telegram.bot.main.batcher", spec=True) as mock_batcher:
...
async def test_post_init(patch_db):
    ...
    with patch("tmux_ssh_telegram.bot.main.SyncService") as mock_sync_cls:
        ...
        with patch("asyncio.create_task") as mock_create_task:
            ...
            from tmux_ssh_telegram.bot import main as bot_main
...
def test_main():
    with patch("tmux_ssh_telegram.bot.main.ApplicationBuilder") as mock_builder:
        ...
        from tmux_ssh_telegram.bot.main import main
        main()
        
        mock_app.run_polling.assert_called_once()

def test_entry_point():
    with patch("tmux_ssh_telegram.bot.main.main") as mock_main:
        # Simulate normal run
        with patch("tmux_ssh_telegram.bot.main.__name__", "__main__"):
            # We can't easily trigger the block without re-importing or executing
            pass

def test_keyboard_interrupt():
    with patch("tmux_ssh_telegram.bot.main.main", side_effect=KeyboardInterrupt):
        with patch("tmux_ssh_telegram.bot.main.logger") as mock_logger:
